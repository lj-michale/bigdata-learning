...
object Grade {
  ...
  // SQL请换成你自己的
  val prepareStatement: PreparedStatement = connection.prepareStatement("""insert into GIS_SCORERECORD(ID,......) values(SEQ_GIS_SCORERECORD.nextval,?.....)""")
  var num = 0
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.sql("""你的SQL""").rdd.collect.foreach { rows => { prepareStatement.setBigDecimal(1, rows.getDecimal(0))
